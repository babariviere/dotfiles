...
local palette = setmetatable({}, {
  __index = function(_, key)
    ...
    if key:sub(1, #'color_') == 'color_' then return table[key] end
    return table[key][1]
  end
})
...
gls.left[14] = {Space = {provider = function() return ' ' end}}
...
local function statusline_lsp()
  ...
  for _, msg in ipairs(buf_messages) do
    ...
    if msg.progress then
      ...
      if msg.spinner then
        contents = lsp_config.spinner_frames[(msg.spinner %
                     #lsp_config.spinner_frames) + 1] .. ' ' .. contents
      end
  ...
  return symbol .. (vim.b.lsp_client_name or '') .. ' ' ..
           lsp_config.indicator_ok
end
...
gls.right[2] = {
  LspStatus = {
    ...
    condition = function()
      if #vim.lsp.buf_get_clients() == 0 then return false end
      return true
    end,
    separator = ' | ',
